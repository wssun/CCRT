...
class LatentDiffusion(DDPM):
    ...
    def estimate_fisher(self, dataset, word, sample_size, batch_size=32):
        #todo 试一下 Variable()全换成.detach()
        loglikelihoods = []
        for i in range(0, len(dataset), batch_size):
            batch_x = torch.stack([sample[0].view(batch_size, -1) for sample in dataset[i:i + batch_size]])
            batch_y = torch.stack([sample[1].view(batch_size, -1) for sample in dataset[i:i + batch_size]])
            batch_x = Variable(batch_x).cuda()
            batch_y = Variable(batch_y).cuda()
            loglikelihoods.append(
                F.log_softmax(self(batch_x,word), dim=1)[range(batch_size), batch_y.data]
            )
